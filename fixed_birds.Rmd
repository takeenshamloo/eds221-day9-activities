...
unique(birds_1$species_id) # Fix #1
...
```{r, include = FALSE}
birds_df <- birds_1 %>% 
  left_join(taxalist_1, by = "species_id") # Fix #2 'species' to 'species_id'
```
...
bird_totals_by_park <- birds_df %>% 
  left_join(sites_1, by = "site_id") %>% 
  group_by(park_name) %>% 
  summarize(total_count = sum(bird_count, na.rm = TRUE)) %>%   
  slice_max(total_count, n = 15) %>% 
  mutate(park_name = forcats::fct_reorder(park_name, total_count))
```

This code creates a plot of the top 15 parks by total bird count. 
```{r}
library(ggplot2)
ggplot2::ggplot(data = bird_totals_by_park, aes(x = total_count, y = park_name)) +
  geom_col() +
  labs(x = "Total bird count",
       y = "Park name") + # Fix #4 Removed pipe
  theme_minimal()
```
...
common_birds <- birds_df %>% 
  left_join(sites_1) %>% # changed semi join to left join
...
```{r}
ggplot(data = selected_parks) +
  geom_col(aes(y = common_name, x = total_count)) +
